...
const parseScores = (rows: Row[], issues: ImportIssue[]) => {
  ...
  for (let index = headerRow + 1; index < rows.length; index += 1) {
    ...
    if (total < 0 || (!rawExam.includes("入口") && total > 750)) outOfRangeRows += 1;
...
export async function parseGradeWorkbook(file: File): Promise<GradeDataset> {
  ...
  const baseRows = XLSX.utils.sheet_to_json<Row>(workbook.Sheets[baseSheetName], { header: 1, raw: true, defval: null }), parsedThresholds = parseThresholds(baseRows, issues), scoreResult = parseScores(baseRows, issues), itemResult = parseItems(workbook, issues), exams = orderExams(uniq(scoreResult.scores.map((score) => score.exam))), thresholds = parsedThresholds.filter((threshold) => exams.includes(threshold.exam));
