import { classifyScore } from "./score-validation";
...
import { getClassProfile, normalizeExam, relevantSubjects, orderExams } from "./class-config";
import type { DataCapability, DataProfile, FieldMatch, GradeDataset, ImportIssue, ItemResponse, QuestionMeta, StudentScore, SubjectName, Threshold, Track, ScoreIssue, ScoreConflict } from "./types";
...
const parseScores = (rows: Row[], issues: ImportIssue[], sheetName: string) => {
  const headerRow = findScoreHeader(rows), scoreIssues: ScoreIssue[] = [], scoreConflicts: ScoreConflict[] = [];
  if (headerRow === null) return { scores: [] as StudentScore[], preferredSchool: "未识别学校", fieldMatches: [] as FieldMatch[], headerRow: null, reconstructedTotals: 0, skippedRows: 0, scoreIssues, scoreConflicts };
  ...
  for (let index=headerRow+1; index<rows.length; index++) {
    const row=rows[index], rawExam=columns.exam===null ? "" : text(row[columns.exam]), classValue=columns.classNo===null ? null : num(row[columns.classNo]), name=columns.name===null ? "" : text(row[columns.name]), school=(columns.school===null ? "" : text(row[columns.school])) || "未标注学校";
    if (!rawExam && !name && classValue===null) continue;
    const recordIssue = (field: string, state: ScoreIssue["state"], value: unknown, column?: number) => scoreIssues.push({school,exam:normalizeExam(rawExam),classNo:classValue,name,field,state,rawValue:text(value).slice(0,100),source:{sheet:sheetName,row:index+1,column:column===undefined ? undefined : column+1}});
    if (!rawExam || rawExam.startsWith("#") || !name || name.startsWith("#") || classValue===null || !Number.isInteger(classValue) || classValue<=0) { skippedRows++; recordIssue("身份字段","invalid",`${rawExam} / ${classValue ?? "空白"} / ${name}`); continue; }
    const classNo=classValue, profile=getClassProfile(classNo,rawExam), subjects: StudentScore["subjects"]={}, subjectStates: StudentScore["subjectStates"]={};
    relevantSubjects(profile).forEach(subject=>{
      const sourceSubject: SubjectName=classNo===7 && subject==="日语" ? "英语" : classNo===9 && subject==="地理" ? "生物" : subject;
      const column=columns.subjects[sourceSubject] ?? columns.subjects[subject];
      const cell=classifyScore(column===undefined ? null : row[column], rawExam.includes("入口") ? undefined : ["语文","数学","英语","日语"].includes(subject) ? 150 : 100);
      subjectStates[subject]=cell.state;
      if (cell.value!==null) subjects[subject]=cell.value;
      else recordIssue(subject,cell.state,column===undefined ? null : row[column],column);
    });
    const totalCell=classifyScore(columns.total===null ? null : row[columns.total], rawExam.includes("入口") ? undefined : 750);
    // Never replace this workbook's source total with a subtotal of incomplete subjects.
    if (totalCell.value===null) { skippedRows++;recordIssue("总分",totalCell.state,columns.total===null ? null : row[columns.total],columns.total ?? undefined);continue; }
    rawScores.push({exam:normalizeExam(rawExam),rawExam,school,classNo,name,track:profile.track,classType:profile.type,combination:profile.combination,total:totalCell.value,totalSource:"source",source:{sheet:sheetName,row:index+1},subjectStates,subjects,cityRank:columns.cityRank===null ? null : num(row[columns.cityRank]),schoolRank:columns.schoolRank===null ? null : num(row[columns.schoolRank])});
  ...
  const groups=new Map<string,StudentScore[]>();
  rawScores.filter(s=>s.school===preferredSchool).forEach(s=>{const key=`${s.exam}::${s.classNo}::${s.name}`;groups.set(key,[...(groups.get(key)??[]),s]);});
  const scores: StudentScore[]=[], fingerprint=(s:StudentScore)=>JSON.stringify([s.rawExam,s.total,Object.entries(s.subjects).sort(),s.subjectStates]);
  let duplicateCount=0, conflictCount=0;
  groups.forEach((candidates,key)=>{
    if(candidates.length===1){scores.push(candidates[0]);return;}
    const same=candidates.every(s=>fingerprint(s)===fingerprint(candidates[0]));
    if(same){scores.push(candidates.at(-1)!);duplicateCount+=candidates.length-1;scoreConflicts.push({key,candidates,resolution:"rank-only"});}
    else {conflictCount++;scoreConflicts.push({key,candidates,resolution:"excluded"});candidates.forEach(s=>scoreIssues.push({school:s.school,exam:s.exam,classNo:s.classNo,name:s.name,field:"重复成绩",state:"identity-conflict",rawValue:`总分${s.total}`,source:s.source!}));}
  });
  if (skippedRows) issues.push({level:"warning",module:"成绩",affectedCount:skippedRows,message:`${skippedRows}行缺少有效身份或源总分，未进入正式统计；可在源行质检中定位，不再用部分科目小计补总分。`});
  if(duplicateCount) issues.push({level:"info",module:"成绩",affectedCount:duplicateCount,message:`${duplicateCount}条重复行的总分及单科完全一致，合并为一条并保留最后排名；来源记录可核对。`});
  if(conflictCount) issues.push({level:"warning",module:"成绩",affectedCount:conflictCount,message:`${conflictCount}组同考试、同班、同名成绩存在冲突，保留全部来源并暂不计入统计；请核对源表后重导入。`});
  return { scores,preferredSchool,fieldMatches,headerRow,reconstructedTotals:0,skippedRows,scoreIssues:scoreIssues.filter(i=>i.school===preferredSchool),scoreConflicts };
};
...
const parseItems = (workbook: WorkBook, issues: ImportIssue[]) => {
  ...
  SUBJECTS.filter((subject) => subject !== "日语").forEach((subject) => { const sheetName = sheetForSubject(workbook, subject); if (!sheetName) return; const rows = XLSX.utils.sheet_to_json<Row>(workbook.Sheets[sheetName], { header: 1, raw: true, defval: null, range: { s: { r: 0, c: 0 }, e: { r: XLSX.utils.decode_range(workbook.Sheets[sheetName]["!ref"] ?? "A1").e.r, c: MAX_ITEM_COLUMNS } } }), metaColumns = new Map<string, number[]>();
    ...
    rows.forEach((row) => { const rawExam = text(row[0]), classValue = num(row[1]), name = text(row[2]); if (!rawExam || classValue === null || !name || normalized(name) === "题号") return; const exam = normalizeExam(rawExam), columns = metaColumns.get(exam); if (!columns?.length) return; const scores = columns.map((column) => num(row[column])); if (!scores.some((score) => score !== null)) return; const classNo = Math.trunc(classValue), normalizedSubject: SubjectName = subject; itemResponses.push({ subject: normalizedSubject, exam, classNo, name, scores }); });
  });
  // An observed maximum is not the question's full mark. Keep missing marks unknown.
...
const buildProfile = (scores: StudentScore[], thresholds: Threshold[], itemResponses: ItemResponse[], exams: string[], fieldMatches: FieldMatch[], headerRow: number | null, reconstructedTotals: number, skippedRows: number): DataProfile => {
  const expected = scores.reduce((sum, score) => sum + relevantSubjects(getClassProfile(score.classNo, score.rawExam)).length, 0), present = scores.reduce((sum, score) => sum + relevantSubjects(getClassProfile(score.classNo, score.rawExam)).filter((subject) => typeof score.subjects[subject] === "number").length, 0), subjectCompleteness = expected ? present / expected : 0;
...
export async function parseGradeWorkbook(file: File): Promise<GradeDataset> {
  ...
  // Error cells carry numeric codes; preserve errors before score conversion.
  for (const sheet of Object.values(workbook.Sheets)) for (const address of Object.keys(sheet)) { const cell=sheet[address]; if (cell && typeof cell === "object" && cell.t === "e") { cell.v=cell.w || "#ERROR!"; cell.t="s"; } }
  const baseRows = XLSX.utils.sheet_to_json<Row>(workbook.Sheets[baseSheetName], { header: 1, raw: true, defval: null }), parsedThresholds = parseThresholds(baseRows, issues), scoreResult = parseScores(baseRows, issues, baseSheetName), itemResult = parseItems(workbook, issues), exams = orderExams(uniq(scoreResult.scores.map((score) => score.exam))), thresholds = parsedThresholds.filter((threshold) => exams.includes(threshold.exam));
