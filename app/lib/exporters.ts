  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        paragraph(`本次纳入${summary.count}人，平均分${summary.average.toFixed(1)}，中位数${summary.median.toFixed(1)}；一本/特控上线${metric(summary.topCount)}人（${percentage(summary.topRate)}），本科上线${metric(summary.undergraduateCount)}人（${percentage(summary.undergraduateRate)}）。一本临界${summary.topCriticalCount}人，本科临界${summary.undergraduateCriticalCount}人。`),
        table(["指标", "数值", "口径"], [["参考人数", String(summary.count), "有效总分"], ["平均分", summary.average.toFixed(1), "总分"], ["中位数", summary.median.toFixed(1), "总分"], ["一本上线", `${metric(summary.topCount)} / ${percentage(summary.topRate)}`, "已识别分数线"], ["本科上线", `${metric(summary.undergraduateCount)} / ${percentage(summary.undergraduateRate)}`, "已识别分数线"]]),
        new Paragraph({ text: "二、成绩分布与分层", heading: HeadingLevel.HEADING_1 }),
        table(["区间", "人数", "占比"], model.distribution.map((item) => [item.label, String(item.count), `${percentage(item.rate)}`])),
        table(["分层", "人数", "占比", "行动意图"], model.segments.map((item) => [item.label, String(item.count), `${percentage(item.rate)}`, item.intent])),
        ...
        table(["质检项", "结果"], [["综合识别置信度", `${percentage(model.quality.confidence)}`], ["学科完整度", `${percentage(model.quality.subjectCompleteness)}`], ["分数线完整度", `${percentage(model.quality.thresholdCompleteness)}`], ["小题覆盖度", `${percentage(model.quality.itemCoverage)}`], ["重建总分", String(model.quality.reconstructedTotals)], ["可用模块", model.quality.availableModules.join("、") || "—"]]),
