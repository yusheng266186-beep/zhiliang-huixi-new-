...
}).filter((student) => student.criticalTiers.length > 0).sort((a, b) => Math.max(b.topDiff ?? -999, b.undergraduateDiff ?? -999) - Math.max(a.topDiff ?? -999, a.undergraduateDiff ?? -999));
...
export const segmentSummary = (dataset: GradeDataset, exam: string, track: Track | "全部", classNo: number | "全部"): ScoreSegment[] => {
  ...
  rows.forEach((student) => { const threshold = getThreshold(dataset, exam, student.track), top = threshold?.topTotal, undergraduate = threshold?.undergraduateTotal; let id: ScoreSegment["id"] = "unclassified"; if (typeof top === "number" && student.total >= top + 30) id = "high"; else if (typeof top === "number" && student.total >= top) id = "top"; else if (typeof top === "number" && student.total >= top - 20) id = "top-critical"; else if (typeof undergraduate === "number" && student.total >= undergraduate) id = "undergraduate"; else if (typeof undergraduate === "number" && student.total >= undergraduate - 20) id = "undergraduate-critical"; else if (typeof undergraduate === "number") id = "foundation"; buckets.set(id, [...(buckets.get(id) ?? []), student]); });
