...
export const classSummaries = (dataset: GradeDataset, exam: string, track: Track | "全部"): ClassSummary[] => {
  ...
  return [...groups.entries()].sort((a, b) => a[0] - b[0]).map(([classNo, students]) => {
    ...
    return { classNo, label: profile.label, track: profile.track, type: profile.type, count: students.length, average: average(students.map((student) => student.total)), topCount, undergraduateCount, topRate: summary.topRate, undergraduateRate: summary.undergraduateRate, subjectAverages };
  });
};
...
export const classBenchmarks = (dataset: GradeDataset, exam: string, track: Track | "全部"): ClassBenchmark[] => classSummaries(dataset, exam, track).map((item, _, summaries) => {
  ...
  const peerAverage = average(peers.map((candidate) => candidate.average)), peerTopRate = average(peers.map((candidate) => candidate.topRate).filter(Number.isFinite)), peerUndergraduateRate = average(peers.map((candidate) => candidate.undergraduateRate).filter(Number.isFinite));
...
export const subjectSummaries = (dataset: GradeDataset, exam: string, track: Track | "全部", classNo: number | "全部"): SubjectSummary[] => {
  ...
  return subjects.map((subject) => {
    ...
    return { subject, topEligible, undergraduateEligible, count: values.length, average: average(values), max: values.length ? Math.max(...values) : 0, topEffectiveCount: topEligible ? topEffectiveCount : Number.NaN, topEffectiveRate: topEligible ? topEffectiveCount / topEligible : Number.NaN, topEffectiveLine, undergraduateEffectiveCount: undergraduateEligible ? undergraduateEffectiveCount : Number.NaN, undergraduateEffectiveRate: undergraduateEligible ? undergraduateEffectiveCount / undergraduateEligible : Number.NaN, undergraduateEffectiveLine, effectiveCount: undergraduateEligible ? undergraduateEffectiveCount : Number.NaN, effectiveRate: undergraduateEligible ? undergraduateEffectiveCount / undergraduateEligible : Number.NaN, effectiveLine: undergraduateEffectiveLine };
  });
};
...
export const buildExecutiveInsights = (dataset: GradeDataset, exam: string, track: Track | "全部", classNo: number | "全部"): ExecutiveInsight[] => {
  ...
  if (weakest) insights.push({ id: "subject", tone: weakest.undergraduateEffectiveRate < .55 ? "attention" : "warn", title: "学科短板", finding: `${weakest.subject}本科有效率${(weakest.undergraduateEffectiveRate * 100).toFixed(1)}%，在当前可比学科中最低。`, action: dataset.itemResponses.some(r => r.exam === exam && r.subject === weakest.subject && rows.some(s => s.classNo === r.classNo && s.name === r.name)) ? "下钻本次可用小题与知识点，定位共性问题。" : "本次该学科未提供小题明细；先按成绩与距线分差制定辅导计划。" });
...
export const onlineSummary = (dataset: GradeDataset, exam: string, rows: StudentScore[]) => {
  ...
  const topCount = top.length ? top.filter(r => r.total >= getThreshold(dataset,exam,r.track)!.topTotal!).length : Number.NaN;
  const undergraduateCount = undergraduate.length ? undergraduate.filter(r => r.total >= getThreshold(dataset,exam,r.track)!.undergraduateTotal!).length : Number.NaN;
  return { topCount, undergraduateCount, topEligible: top.length, undergraduateEligible: undergraduate.length, topRate: top.length ? topCount/top.length : Number.NaN, undergraduateRate: undergraduate.length ? undergraduateCount/undergraduate.length : Number.NaN };
};
