import { ascendingMetric } from "./metrics";
...
import type { GradeDataset, SubjectName, Track, StudentScore, ScoreIssue } from "./types";
...
export type QualityReportModel = {
  ...
  trend: Array<{ exam: string; count: number; average: number; topCount: number | null; undergraduateCount: number | null }>;
  quality: {
    confidence: number;
...
const recommendationsFor = (model: Pick<QualityReportModel, "insights" | "segments" | "subjects" | "knowledge" | "classes" | "critical">): string[] => {
  ...
  const weakest = model.subjects.filter(s => Number.isFinite(s.undergraduateEffectiveRate)).sort((a, b) => ascendingMetric(a.undergraduateEffectiveRate, b.undergraduateEffectiveRate))[0];
...
export function buildQualityReport(dataset: GradeDataset, options: { exam: string; track: Track | "全部"; classNo: number | "全部"; reportType: ReportType; subject?: SubjectName }): QualityReportModel {
  ...
  const knowledgeSubject = subject ?? subjects.filter(s=>Number.isFinite(s.undergraduateEffectiveRate)).sort((a, b) => ascendingMetric(a.undergraduateEffectiveRate, b.undergraduateEffectiveRate))[0]?.subject ?? "语文";
  ...
  const quality = { confidence: profile?.overallConfidence ?? 0, subjectCompleteness: rows.length ? rows.reduce((n,r)=>n+Object.values(r.subjects).filter(v=>typeof v === "number").length,0)/rows.reduce((n,r)=>n+relevantSubjects(getClassProfile(r.classNo,r.rawExam)).length,0) : 0, thresholdCompleteness: profile?.thresholdCompleteness ?? 0, itemCoverage: rows.length ? rows.filter(s => dataset.itemResponses.some(r=>r.exam===exam && r.classNo===s.classNo && r.name===s.name)).length/rows.length : 0, reconstructedTotals: profile?.reconstructedTotals ?? 0, warnings: warningCount, errors: errorCount, availableModules };
  ...
  const model: QualityReportModel = {
    students: rows, audit: (dataset.scoreIssues ?? []).filter(i => i.exam === exam && (classNo === "全部" || i.classNo === classNo) && (track === "全部" || (i.classNo !== null && getClassProfile(i.classNo).track === track))), reportType, exam, track, classNo, subject, title: `${exam} · ${reportType}`, focusStatement: focusByType[reportType], scope: `${track}${classNo === "全部" ? " · 全部班级" : ` · ${classNo}班`}`, generatedAt: new Date().toISOString(), summary, stats, distribution: distributionBins(rows.map((row) => row.total)), segments, classes, subjects, critical, knowledge, insights, recommendations: [], trend, quality, methodology: [`本次一本指标可计算${online.topEligible}人，本科指标可计算${online.undergraduateEligible}人；比率分母仅包含有适用分数线且总分非重建者。`, "历次原始均分不等于教学增值；考试难度、满分及赋分规则需核验。","以学生基础表中可识别的考试、班级、姓名和总分作为主键。", "总分仅采用有效源表总分；缺失或异常总分不以部分学科重建。同考试、同班、同名成绩冲突时保留来源并暂不纳入统计。", "学科均值只使用该学科的有效成绩，缺失学科不按0分进入分母。", "上线口径按考试与类别匹配一本/特控线、本科线；未配置分数线的指标显示为不可用。", "班级对标优先按类别与班型建立同类组，样本不足时退回同类别比较。", "小题得分率按可识别满分计算；源表缺失满分时不计算正式得分率，也不生成补弱结论。"], sourceName: dataset.sourceName,
  };
