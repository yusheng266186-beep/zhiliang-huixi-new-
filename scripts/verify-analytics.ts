import { ascendingMetric } from "../app/lib/metrics";
...
const knowledgeSubject = [...subjects].sort((a, b) => ascendingMetric(a.undergraduateEffectiveRate,b.undergraduateEffectiveRate))[0]?.subject ?? "语文";
...
if (!subjects.length || insights.length < 3) throw new Error("rich analysis modules are incomplete");
...
reports.forEach((report) => {
  if (report.summary.count !== rows.length || report.segments.reduce((sum, segment) => sum + segment.count, 0) !== rows.length || report.methodology.length < 6 || report.recommendations.length < 3) throw new Error(`${report.reportType} report assertions failed`);
});
